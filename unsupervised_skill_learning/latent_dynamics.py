...
class LatentDynamics:
  ...
  # dynamics graph with separate pipeline for latents and timesteps
  def _graph_with_separate_latent_pipe(self, timesteps, actions):
    latent_out = actions
    with tf.compat.v1.variable_scope('action_pipe'):
      for idx, layer_size in enumerate((self._fc_layer_params[0] // 2,)):
        latent_out = tf.compat.v1.layers.dense(
            latent_out,
            layer_size,
    ...
    # out = tf.compat.v1.layers.flatten(tf.einsum('ai,aj->aij', ts_out, latent_out))
    out = tf.concat([ts_out, latent_out], axis=1)
